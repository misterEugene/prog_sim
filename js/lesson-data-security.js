...
и нажми зелёную кнопку **✓ Я выполнил этот шаг**.`,
    ...
    {
      manual: true,
      title: `💉 Дыра №1а: XSS в комментарии - 🔴 сломай`,
      time: `12 мин`,
...
**Где дыра:** комментарии к товару, имя автора и уголок пользователя выводятся через \`innerHTML\` - браузер исполняет вставленный HTML.`,
      actionMd: `Сейчас только ломаем (🔴). Чинить будем на следующем шаге.`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. Зарегистрируйся (логин vasya, пароль 12345678) и войди.
2. Открой комментарии любого товара.
3. Отправь в комментарий ловушку \`<img src=x onerror="alert('XSS')">\`.
4. Повтори с кражей \`currentUser\`.
5. Проверь, что ловушка срабатывает и у другого пользователя (stored XSS).
...
**8. 🧪 XSS - это не только окошки. Попробуй сам «испортить» страницу.** Раз сайт
вставляет комментарий как настоящий HTML, туда можно положить **любой** код, а не
только \`alert\`. Войди снова под своим первым логином (\`vasya\`), открой любой товар и
отправляй эти комментарии по очереди, каждый раз глядя на страницу:
- \`<marquee>Тут был хакер</marquee>\` - по странице поползёт **бегущая строка** (так в старину «взламывали» сайты ради баловства);
- \`<h1 style="color:red">ВЗЛОМАНО</h1>\` - на странице появится **чужой красный заголовок**, будто сайт «переписали»;
- \`<button onclick="alert('нажми меня')">Приз!</button>\` - появится **фальшивая кнопка**, а по клику выполнится чужой код. Так жертву заманивают нажать.

💡 Вывод: через одну дыру XSS злоумышленник может подменить вид страницы, подсунуть
фальшивые кнопки и формы (например, «введите пароль») - и всё это будет выглядеть как
часть настоящего сайта. Поэтому XSS - одна из самых опасных дыр в вебе.`,
      hintMd: `Ловушку вставляй именно в поле комментария и жми «Отправить». \`alert(...)\` - это всплывающее окошко: выскочило - значит, чужой код выполнился.`,
      doneMd: `✅ **Готово, когда** у тебя выскочило окошко \`alert\` из комментария, ты увидел кражу имени и проверил, что ловушка срабатывает и у другого пользователя.`,
    },
    {
      manual: true,
      title: `💉 Дыра №1а: XSS в комментарии - 🟢 почини`,
      time: `10 мин`,
      goalMd: `**Цель:** закрыть XSS в тексте комментария - выводить его через \`textContent\` (как обычный текст, а не как HTML).`,
      actionMd: `Почини код в **main.js** (🟢) и проверь (✅).`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. Скопируй безопасную версию функции \`renderComments\` из рамки ниже (Ctrl+C).
2. Открой вкладку main.js.
3. Найди функцию \`renderComments\` через Ctrl+F.
4. Выдели её целиком, удали и сразу вставь скопированное (Ctrl+V).
5. Нажми ▶ Запустить и проверь, что ловушка больше не срабатывает.

---

🔧 **🟢 ПОЧИНИ (файл main.js).**

**Действие 1. Скопируй новый код.** Ниже в рамке лежит безопасная версия функции
\`renderComments\` - она выводит текст комментария через \`textContent\` (то есть как
**обычный текст**, а не как HTML). Имя автора мы пока **намеренно** оставляем как
есть - к нему вернёмся чуть позже. Выдели весь код в рамке мышкой (зажми левую кнопку
мыши перед словом \`function\` и протяни до самой последней \`}\`) и нажми **Ctrl+C** -
код запомнится в буфере обмена.

⚠️ **Копируй именно сейчас, до того как что-то удалять.** Пока старый код на месте,
его легко найти. А если сначала удалить, а потом идти за новым кодом - вернуться будет
некуда: место пропало.
...
**Действие 2. Открой нужный файл.** В средней колонке «Код» сверху есть три вкладки.
Наведи курсор на вкладку **main.js** (третья) и щёлкни левой кнопкой мыши - откроется
код этого файла.

**Действие 3. Найди нужную функцию с именем \`renderComments\`.** Нажми на клавиатуре **Ctrl+F** (зажми клавишу \`Ctrl\` и, не
...
**Действие 4. Выдели всю функцию целиком и удали.** Функция начинается со строки
...
**Действие 5. Вставь скопированный код.** Курсор уже стоит ровно там, где ты удалил
старую функцию, - никуда не уходи и просто нажми **Ctrl+V**. На месте старой функции
появится безопасная версия из рамки выше.
...
закрыта! 🛡️`,
      hintMd: `\`textContent\` вставляет строку как текст - браузер её не исполняет. Имя автора в этой версии пока намеренно остаётся как есть - вернёмся к нему позже.`,
      doneMd: `✅ **Готово, когда** после ▶ Запустить ловушка \`<img src=x onerror=…>\` в комментарии показывается как обычный текст, а окошко не выскакивает.`,
    },
    {
      manual: true,
      title: `💉 Дыра №1б: XSS в имени пользователя - 🔴 сломай`,
      time: `8 мин`,
      goalMd: `**Цель:** убедиться, что XSS прячется не только в комментариях, но и в **имени пользователя**.

**Где дыра:** имя в уголке «👤 …» и в приветствии выводится через \`innerHTML\`.`,
      actionMd: `Снова только ломаем (🔴).`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. Выйди из аккаунта и открой Регистрацию.
2. Впиши ловушку \`<img src=x onerror="alert('XSS в профиле')">\` ВМЕСТО логина.
3. Зарегистрируйся и войди этим аккаунтом.
4. Посмотри, как окошко выскакивает прямо из имени.
...
**🧪 Проверь ещё раз - другой ловушкой.** Чтобы убедиться, что дело именно в имени,
а не в «магическом» слове, выйди и зарегистрируй ещё один аккаунт, но логином впиши
другую ловушку: \`<b onmouseover="alert('навёл мышку!')">наведи на меня</b>\`. Войди им
и **наведи курсор** на своё имя в правом верхнем углу - окошко выскочит от одного
движения мышкой, без всякого клика. Значит, опасна не конкретная строка, а сам факт,
что имя вставляют как HTML.

🤔 **Подумай:** в скольких местах сайт показывает имя пользователя? (Подсказка: уголок
«👤 …» вверху и приветствие «Привет, …» после входа.) Держи это число в голове - на
следующем шаге все эти места придётся чинить, и ни одно нельзя пропустить.`,
      hintMd: `Ловушку впиши именно в поле логина при регистрации, и тот же логин - при входе. Имя показывается в нескольких местах, поэтому окошек может быть несколько подряд.`,
      doneMd: `✅ **Готово, когда** при входе под «вредным» логином окошко \`alert\` выскакивает из имени в уголке и в приветствии.`,
    },
    {
      manual: true,
      title: `💉 Дыра №1б: XSS в имени пользователя - 🟢 почини`,
      time: `10 мин`,
      goalMd: `**Цель:** закрыть XSS в имени пользователя в двух местах - уголок «👤 …» и приветствие после входа.`,
      actionMd: `Почини оба места в **main.js** (🟢) и проверь (✅).`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. Скопируй безопасную версию функции \`renderUserBox\` из рамки ниже (Ctrl+C), потом
   найди её в main.js, удали старую и вставь скопированное (Ctrl+V).
2. Найди строку \`loginMsg.innerHTML = …\` и замени слово \`innerHTML\` на \`textContent\`.
3. Нажми ▶ Запустить.
4. Войди под «вредным» логином и проверь, что окошко не выскакивает.
...
**а) Уголок пользователя** (имя «👤 …» в правом верхнем углу сайта).

**Действие 1. Скопируй новый код.** Ниже в рамке - безопасная версия функции
\`renderUserBox\`. Выдели её мышкой целиком и нажми **Ctrl+C**. Делай это **до**
удаления старой: пока старый код на месте, его легко найти.
...
**Действие 2. Найди старую функцию.** На вкладке **main.js** нажми **Ctrl+F**,
напечатай \`renderUserBox\` и нажми **Enter**. (Это сочетание встречается в тексте урока
несколько раз, поэтому нажимай **Enter** до тех пор, пока подсветка не перепрыгнет с
текста задания на нужную строку в коде.)

**Действие 3. Выдели её целиком и удали.** Щёлкни в начале строки
\`function renderUserBox() {\`, потом с зажатым **Shift** щёлкни после её закрывающей
\`}\` - вся функция подсветится синим. Нажми **Delete**.

**Действие 4. Вставь скопированное.** Курсор уже стоит на нужном месте - просто нажми
**Ctrl+V**.
...
показывается как безобидный текст, окошко **не выскакивает**. 🛡️ Два места закрыты!`,
      hintMd: `\`renderUserBox\` замени целиком (копируй из задания), а в приветствии меняется всего одно слово: \`innerHTML\` → \`textContent\`.`,
      doneMd: `✅ **Готово, когда** «вредное» имя в уголке и в приветствии показывается как текст, а окошко не выскакивает.`,
    },
    {
      manual: true,
      title: `💉 Дыра №1в: XSS через имя автора - 🔴 сломай`,
      time: `7 мин`,
      goalMd: `**Цель:** найти спрятанную дыру - имя автора рядом с комментарием тоже выводится через \`innerHTML\`.`,
      actionMd: `Снова ломаем (🔴) - теперь без вредного комментария, одним логином.`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. Останься под «вредным» логином из прошлого шага.
2. Открой любой товар.
3. Отправь самый ОБЫЧНЫЙ комментарий (например, \`классный товар\`).
4. Посмотри, как окошко выскакивает из-за имени автора.
...
🤔 **Проверь, что понял, где именно дыра.** На прошлом шаге мы уже сделали текст
комментария безопасным (\`textContent\`), а окошко всё равно выскочило. Как думаешь,
через какое поле теперь просочился код - через **текст** комментария или через **имя
автора**? 👉 Правильно: через **имя автора** - его сайт всё ещё выводит как HTML.
Именно эту, последнюю щёлочку в комментариях мы и закроем на следующем шаге.`,
      hintMd: `Вредного кода в комментарии нет - срабатывает твой ЛОГИН, который сайт показывает как имя автора через \`innerHTML\`.`,
      doneMd: `✅ **Готово, когда** даже безобидный комментарий под «вредным» логином вызывает окошко \`alert\`.`,
    },
    {
      manual: true,
      title: `💉 Дыра №1в: XSS через имя автора - 🟢 почини`,
      time: `6 мин`,
      goalMd: `**Цель:** закрыть последнее место XSS - имя автора в комментарии. После этого XSS закрыт везде.`,
      actionMd: `Почини в **main.js** (🟢) и проверь (✅).`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. В main.js найди строку \`author.innerHTML = c.author + ":";\`.
2. Замени слово \`innerHTML\` на \`textContent\`.
3. Нажми ▶ Запустить.
4. Оставь обычный комментарий «вредным» логином - окошко больше не выскакивает.
...
🛡️ Вот теперь XSS закрыт **во всех местах** - отличная работа!`,
      hintMd: `Меняется одно слово: двойной клик по \`innerHTML\` → напечатай \`textContent\`.`,
...
**Брутфорс** - это когда злоумышленник перебирает все возможные пароли подряд, пока не угадает.`,
      actionMd: `Только ломаем (🔴): подбери слабый пароль перебором в консоли.`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. Открой F12 → Console.
2. Зарегистрируй жертву: логин ровно \`victim\`, пароль из 3–4 цифр.
3. Вставь в консоль код перебора и запусти его.
4. Посмотри, за сколько миллисекунд подобрался пароль.
...
**1. Заведём «жертву» со слабым паролем.** Сделай это через обычную форму. Сначала
убедись, что хэширование из прошлого шага включено и сайт пересобран (кнопка
**▶ Запустить**) - иначе пароль сохранится без хэша и перебор его не найдёт. Затем на
сайте щёлкни **Регистрация**. В поле «Придумай логин» впиши **ровно** \`victim\` (это имя
...
которая отвечает «да» или «нет». Пароли теперь хранятся **в виде хэша**, поэтому на
...
требуют длинный и сложный пароль, а сервер ещё и ограничивает число попыток входа.`,
      ...
      doneMd: `✅ **Готово, когда** ты подобрал короткий цифровой пароль перебором в консоли и увидел время в мс.`,
    },
    {
      manual: true,
...
--------------------------    -----------------------
только цифры (0-9)            пароль из 5-6 цифр
...
его вообще не осталось в коде. Подмена отметки теперь ничего не открывает. 🛡️`,
    ...
    {
      manual: true,
      title: `🔗 Дыра №6: отражённый XSS из ссылки - 🔴 сломай`,
      time: `11 мин`,
...
**Где дыра:** приветствие из ссылки вида \`#welcome=Имя\` показывается на главной через \`innerHTML\`.`,
      actionMd: `Только ломаем (🔴): доставь код прямо через адрес ссылки.`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. В main.js найди функцию \`showWelcome\`.
2. Нажми вверху **🔗 Открыть в новой вкладке** - сайт откроется отдельной вкладкой.
3. В **адресную строку этой вкладки**, сразу после адреса, допиши \`#welcome=<img src=x onerror=alert('Взлом!')>\` и нажми **Enter**.
4. Посмотри, как выскочило окошко «Взлом!» - код пришёл прямо из ссылки.
5. Для сравнения впиши в адрес \`#welcome=Аня\` - увидишь мирное приветствие.
...
**Enter** до тех пор, пока подсветка не перепрыгнет с текста задания на нужную строку
в коде) - это та самая функция, что берёт имя из адреса.

**2. Открой сайт в отдельной вкладке.** В жизни злоумышленник прислал бы жертве
готовую ссылку, и та открыла бы её в обычной вкладке браузера. Сделаем так же. Вверху,
над колонками, найди кнопку **🔗 Открыть в новой вкладке** и щёлкни по ней левой
кнопкой мыши. Откроется **новая вкладка**, где твой сайт занимает весь экран - сам по
себе, без платформы вокруг.

**3. Допиши вредную «ссылку» в адрес.** Перейди в эту новую вкладку. Вверху неё есть
**адресная строка** (там написан адрес страницы). Щёлкни по ней левой кнопкой мыши -
адрес выделится. Нажми клавишу **End** (или щёлкни в самый конец адреса), чтобы курсор
встал **после** адреса, и **в самый конец** допиши вот это:
\`#welcome=<img src=x onerror=alert('Взлом!')>\`
Нажми **Enter**. 💥 Выскочило окошко «Взлом!»? Это и есть **отражённый XSS**: код
пришёл **прямо из адреса ссылки** и выполнился. Представь: злоумышленник кидает такую
ссылку в чат, жертва просто **щёлкает** по ней - и его код уже работает на сайте от её
имени. Нажми **OK**.

ℹ️ **Не пугайся «непонятных значков» в адресе.** После Enter браузер сам заменит
\`<\`, \`>\` и пробелы на «коды» вида \`%3C\`, \`%3E\`, \`%20\` - это нормально, сайт
всё равно их понимает. И ещё: вписывать нужно именно в адрес **этой отдельной вкладки**.
Если дописать \`#welcome=…\` к адресу самой платформы (\`урок_2.html\`, где три колонки) -
**не сработает**: там сайт сидит в защищённом «окошке-песочнице», и адрес до него не
доходит.

**4. Запасной способ - через консоль.** Если возиться со вкладкой не хочется, ту же
«ссылку» можно доставить командой. Нажми **F12**, щёлкни вкладку **Console**, щёлкни в
строку ввода внизу и выполни (Ctrl+V → Enter):
\`location.hash = "welcome=<img src=x onerror=alert('Взлом!')>"\`
Окошко выскочит так же - команда меняет «адрес» прямо сейчас.

**5.** Для сравнения впиши **безобидное** имя - в адрес отдельной вкладки допиши
\`#welcome=Аня\` (или в консоли \`location.hash = "welcome=Аня"\`): увидишь мирное
«С возвращением, Аня!». Значит, опасна именно вставка через \`innerHTML\`.`,
      hintMd: `Главный способ: «🔗 Открыть в новой вкладке» → в адресную строку ТОЙ вкладки допиши в конец \`#welcome=<img src=x onerror=alert('Взлом!')>\` → Enter. Браузер закодирует символы (\`%3C\` и т.п.) - это нормально, сайт их раскодирует. К адресу самой платформы (\`урок_2.html\`) дописывать бесполезно - там сайт в песочнице. Запасной способ - та же строка через F12 → Console: \`location.hash = "welcome=…"\`. Чинится тем же приёмом, что и обычный XSS: \`innerHTML\` → \`textContent\`.`,
      doneMd: `✅ **Готово, когда** ты дописал \`#welcome=<img …>\` в адрес отдельной вкладки (или выполнил команду в консоли), окошко \`alert\` выскочило, а с обычным именем (\`#welcome=Аня\`) показалось мирное приветствие.`,
    },
    {
      manual: true,
      title: `🔗 Дыра №6: почини приветствие - 🟢 почини`,
      time: `9 мин`,
      goalMd: `**Цель:** закрыть отражённый XSS - выводить приветствие через \`textContent\`.`,
      actionMd: `Почини (🟢) и проверь (✅).`,
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. В main.js найди и замени функцию \`showWelcome\` на безопасную из задания.
2. Нажми ▶ Запустить.
3. Снова выполни вредную команду - окошко больше не выскакивает.
...
**Действие 1. Скопируй новый код.** Ниже в рамке - безопасная версия функции
\`showWelcome\`. Выдели её мышкой целиком и нажми **Ctrl+C**. Делай это **до** удаления
старой: пока старый код на месте, его легко найти.
...
**Действие 2. Найди старую функцию.** Нажми **Ctrl+F**, напечатай \`showWelcome\` и жми
**Enter** до тех пор, пока подсветка не перепрыгнет с текста задания на нужную строку
в коде.

**Действие 3. Выдели её целиком и удали.** Щелчок в начало строки
\`function showWelcome(name) {\`, затем Shift+щелчок после её закрывающей \`}\` - вся
функция подсветится синим. Нажми **Delete**.
...
**F12 → Console** и выполни команду, которая разом выведет **всю** «кладовку» браузера:
\`console.log(JSON.stringify(localStorage, null, 2))\`
Пробеги глазами вывод: нет ли там ещё какого-нибудь подозрительного ключа со словом
\`flag\`, \`secret\`, \`token\` или \`admin\`? В нашем учебном сайте флагов ровно три, но
...
lesson.stepFixes = [
	...
	/*  2. Дыра №1а XSS коммент 🔴     */ [],
	/*  3. Дыра №1а XSS коммент 🟢     */ [
	...
	/*  6. Дыра №1в XSS автор 🔴       */ [],
	/*  7. Дыра №1в XSS автор 🟢       */ [
		{ file: "js", op: "replaceStr", find: 'author.innerHTML = c.author + ":";', replace: 'author.textContent = c.author + ":";' },
	],
	/*  8. Дыра №2 пароль+обход 🔴     */ [],
	/*  9. Дыра №2 пароль+обход 🟢     */ [
	...
	/* 10. Дыра №3 открытый текст 🔴   */ [],
	/* 11. Дыра №3 хэширование 🟢      */ [
