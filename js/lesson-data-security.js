...
2. В F12 → Console выполни \`location.hash = "welcome=<img src=x onerror=alert('Взлом!')>"\`.
3. Посмотри, как код из адреса выполнился.
4. Для сравнения выполни \`location.hash = "welcome=Аня"\`.
...
ссылку. У нас «адрес» сайта можно задать командой. Нажми **F12**, щёлкни вкладку
**Console**, щёлкни в строку ввода (прямо здесь, на платформе) и выполни (Ctrl+V → Enter):
\`location.hash = "welcome=<img src=x onerror=alert('Взлом!')>"\`
💥 Прямо в превью справа выскочило окошко «Взлом!». Код пришёл **прямо из адреса
ссылки** и выполнился - это **отражённый XSS**. Представь: злоумышленник кидает такую
ссылку в чат, жертва щёлкает по ней - и его код работает на сайте от её имени. Нажми
**OK**.

🔗 **Хочешь доставить как настоящую ссылку?** Это работает в **отдельной вкладке**.
Нажми вверху кнопку **🔗 Открыть в новой вкладке** - сайт откроется в чистой вкладке,
сам по себе. Затем щёлкни в **адресную строку этой новой вкладки**, допиши в самый
конец \`#welcome=<img src=x onerror=alert('Взлом!')>\` и нажми **Enter**. 💥 Окошко
выскочит снова - это и есть та самая вредная ссылка, по которой «щёлкнула бы жертва»:
браузер сам раскодирует символы и выполнит код.

⚠️ **Важно, КУДА вписывать.** Это сработает только в адресной строке **отдельной
вкладки** (там сайт открыт сам по себе). Если дописать \`#welcome=…\` к адресу самой
платформы (\`index2.html\`, где три колонки) - **не сработает**: здесь сайт сидит в
защищённом «окошке-песочнице» (iframe), и хэш из адреса платформы до него не доходит.
Поэтому на платформе мы доставляем «ссылку» командой в консоли (пункт 2 выше), а
«как по настоящей ссылке» удобно посмотреть в отдельной вкладке.
...
опасна именно вставка через \`innerHTML\`.`,
      ...
      doneMd: `✅ **Готово, когда** ловушка из \`location.hash\` вызвала окошко \`alert\`, а обычное имя показало мирное приветствие.`,
    },
    {
      manual: true,
