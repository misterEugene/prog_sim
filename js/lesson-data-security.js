...
2. Найди функцию \`renderComments\` через Ctrl+F.
3. Выдели её целиком и удали.
4. Вставь безопасную версию из задания.
...
места - этим и займёмся.`,
    ...
    },
    {
      ...
      taskMd: `📋 **Пункты действия** (что сделать в этом шаге):
1. В main.js замени функцию \`renderUserBox\` на безопасную из задания.
...
\`}\`). **Сначала скопируй** безопасную версию из рамки ниже (выдели её мышкой и нажми
**Ctrl+C**), и только потом нажми **Delete** и сразу вставь скопированное (**Ctrl+V**) -
так ты не потеряешь место, где удалил:
...
Shift+щелчок после её закрывающей \`}\`). **Сначала скопируй** чистую версию из рамки
ниже (выдели мышкой и нажми **Ctrl+C**), и только потом нажми **Delete** и сразу вставь
скопированное (**Ctrl+V**) - так ты не потеряешь место, где удалил. Теперь вход
...
**Shift** и щёлкни сразу после закрывающих \`});\` - это самая последняя строка
обработчика, сразу под строкой \`msg.textContent = "Готово! Теперь войди…";\`.
**Сначала скопируй** новую версию из рамки ниже (выдели мышкой и нажми **Ctrl+C**), и
только потом нажми **Delete** и сразу вставь скопированное (**Ctrl+V**) - так ты не
потеряешь место, где удалил. Эта версия сохраняет в \`users\` хэш пароля, а не сам пароль:
...
(жми **Enter**, пока подсветка не перепрыгнет в код), выдели обработчик целиком: щёлкни
**перед** \`loginBtn\` в начале этой строки, затем с зажатым **Shift** щёлкни **после**
закрывающих \`});\` - это самая последняя строка обработчика, сразу под строкой \`}\`,
которая идёт под строкой \`loginMsg.textContent = "Неверный логин или пароль";\`. Весь
блок выделится синим. **Сначала скопируй** новую версию из рамки ниже (выдели мышкой и
нажми **Ctrl+C**), и только потом нажми **Delete** и сразу вставь скопированное
(**Ctrl+V**) - так ты не потеряешь место, где удалил:
...
с \`<div id="admin-panel"\`, зажми **Shift** и щёлкни сразу после \`</div>\`).
**Сначала скопируй** пустую версию из рамки ниже (выдели мышкой и нажми **Ctrl+C**), и
только потом нажми **Delete** и сразу вставь скопированное (**Ctrl+V**) - так ты не
потеряешь место, где удалил. В этой версии секрета внутри уже нет:
...
\`function showWelcome(name) {\`, затем Shift+щелчок после её закрывающей \`}\`).
**Сначала скопируй** безопасную версию из рамки ниже (выдели мышкой и нажми **Ctrl+C**),
и только потом нажми **Delete** и сразу вставь скопированное (**Ctrl+V**) - так ты не
потеряешь место, где удалил:
