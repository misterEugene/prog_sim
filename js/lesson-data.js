...
к самым краям окна. Маленькое правило, а сайт сразу выглядит аккуратнее.`,
    ...
    {
      // ----- D3: Полировка — приветствие из шагов A2/A3 → тонкая полоска сверху -----
      title: "Приветствие — тонкая полоска сверху",
      time: "7 мин",
      goalMd:
        "**Цель:** аккуратно вписать в сайт тот первый текст, что ты написал руками в начале урока (заголовок «Добро пожаловать…» и абзац под ним). Сейчас он торчит большими буквами над шапкой — превратим его в **тонкую приветственную полоску** наверху.\n\n**Новое:** селектор `body > h1` означает «заголовок, который лежит прямо в `<body>`» — то есть именно тот текст из начала урока, а не заголовки внутри блоков.",
      actionMd:
        "Один CSS-блок. Нажми кнопку **CSS**, потом ▶ Запустить.",
      snippets: {
        css: `/* Приветствие из начала урока → тонкая полоска вверху сайта */
body > h1,
body > p {
	margin: 0;
	background: #2b2b3a;
...
Большой заголовок «Добро пожаловать в МегаМагазин» и абзац под ним превратились
в аккуратную **тёмную полоску над шапкой** — как строка с приветствием на
настоящих сайтах. Теперь начало урока красиво вписалось в магазин, а не мешает ему.

💡 \`body > h1\` и \`body > p\` — это как раз тот текст, что ты печатал руками в
самых первых шагах урока.`,
      hintMd:
        "Вписывать ничего не нужно. Если полоски нет — проверь, что заголовок «Добро пожаловать…» и абзац из первых шагов всё ещё есть в index.html (прямо внутри `<body>`).",
      doneMd:
        "✅ **Готово, когда** вместо большого заголовка вверху видна тонкая тёмная полоска с приветствием над шапкой магазина.",
    },
    {
