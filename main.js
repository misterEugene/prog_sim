...
интернет-магазин: с шапкой, баннером, товарами, отзывами и работающей корзиной.
Магазин будет **твой**: ты сам впишешь название, товары, цены и тексты.

## Как это работает

**Шаг 0 — создай каркас сайта (делается один раз):** открой вкладку
**index.html**, напечатай **!** и нажми клавишу **Tab** — появится основа
страницы (теги \`html\`, \`head\`, \`body\`). Все блоки будут вставляться **внутрь** неё.

**Дальше на каждом шаге повторяй 4 простых действия:**

- 1️⃣ **Вставь блок** кнопками: **HTML** уходит в index.html, **CSS** — в style.css.
- 2️⃣ Открой вкладку с кодом и найди жёлтые метки **[ВПИШИ …]** — это места для твоего текста.
- 3️⃣ **Замени каждую метку** своим текстом. Стирай метку целиком, вместе со скобками **[** и **]**.
- 4️⃣ Нажми **▶ Запустить** — и увидишь, как магазин растёт!

💡 Если после запуска на сайте видна надпись **[ВПИШИ …]** — значит, там ты ещё
не вписал текст. Просто вернись в код и впиши.

Передумал? Нажми **Ctrl+Z** — последнее действие отменится. Поехали! 👇`,
...
- и самое главное — **наполнять сайт своим содержанием**.

Нажми **⬇ Скачать проект** вверху — и твой магазин сохранится в файл. Покажи его друзьям!`,

  hint: "Каждый шаг одинаковый: вставь блок кнопками (HTML, CSS) → открой код и впиши свой текст вместо жёлтых меток [ВПИШИ …] → нажми ▶ Запустить → посмотри результат.",
...
.cart { font-weight: bold; }`,
      ...
      taskMd:
        "🔧 **Впиши свой текст.** Открой вкладку **index.html**, найди жёлтую метку `[ВПИШИ НАЗВАНИЕ МАГАЗИНА]` и замени её на имя своего магазина — например «СуперМаркет», «ИгроМир» или «Вкусняшки».\n\n⭐ **Бонус:** в **style.css** найди у `.site-header` строку `background: #5b3df5;` и поменяй цвет на свой любимый.",
      hintMd:
        "Метку стирай полностью, вместе со скобками `[` и `]`. Цвет в CSS можно написать словом (`teal`, `tomato`, `green`) или кодом вида `#ff8a5b`.",
      doneMd:
        "✅ **Готово, когда** после ▶ Запустить вверху сайта виден твой логотип, меню и «🛒 Корзина: 0» (и нигде не осталось надписи [ВПИШИ …]).",
    },
    {
      // ----- Шаг 2: Баннер -----
...
.banner-btn:hover { background: #4a2fd0; }`,
      ...
      taskMd:
        "🔧 **Впиши свой текст** в **index.html** — три метки:\n\n- `[ВПИШИ ЗАГОЛОВОК СКИДКИ]` → например «Скидки до 50%!»\n- `[ВПИШИ ПОДЗАГОЛОВОК]` → например «Лучшие товары по выгодным ценам»\n- `[ВПИШИ ТЕКСТ КНОПКИ]` → например «Купить сейчас»",
      hintMd:
        "Заголовок — это текст между `<h1>` и `</h1>`, а текст кнопки — между `<button …>` и `</button>`. Меняй только то, что внутри меток.",
      doneMd: "✅ **Готово, когда** на сайте виден яркий баннер с твоим заголовком, подзаголовком и кнопкой.",
    },
    {
      ...
      actionMd:
        "Нажми обе кнопки: **HTML** добавит сетку из четырёх товаров, **CSS** — её стили.",
      snippets: {
...
.add-to-cart:hover { background: #4a2fd0; }`,
      ...
      taskMd:
        "🔧 **Наполни магазин товарами.** В каждой из четырёх карточек три метки — эмодзи (картинка товара), название и цена. Заполни все четыре карточки!\n\nЭмодзи можно скопировать прямо отсюда: 👟 🎧 ⌚ 🎒 📱 🎮 ⚽ 📚 🧸 🎸\n\nНапример: 👟 — Кроссовки — 3990, или 🎧 — Наушники — 2490.",
      hintMd:
        "Цену пиши только числом — значок `₽` уже стоит справа от метки. Эмодзи проще всего выделить в списке выше и скопировать (Ctrl+C), потом вставить (Ctrl+V) вместо метки.",
      doneMd:
        "✅ **Готово, когда** видны четыре карточки с картинками, названиями, ценами и кнопками «В корзину».",
    },
    {
      // ----- Шаг 4: Блок «О нас» -----
      title: "Блок «О нас»",
      time: "10 мин",
      goalMd:
        "**Цель:** рассказать покупателям о магазине — кто вы и чем хороши.\n\n**Новый термин:** `<section>` — это «раздел» страницы. Мы уже встречали его в баннере и товарах, теперь сделаем раздел с рассказом.",
      actionMd:
        "Нажми обе кнопки: **HTML** добавит блок «О нас», **CSS** — его стили.",
      snippets: {
        html: `<!-- Блок "О нас": рассказ о магазине -->
<section class="about" id="about">
  <h2>[ВПИШИ ЗАГОЛОВОК — НАПРИМЕР: О нас]</h2>
  <p>[ВПИШИ 2-3 ПРЕДЛОЖЕНИЯ О ТВОЁМ МАГАЗИНЕ]</p>
</section>`,
...
.about p { font-size: 17px; line-height: 1.6; color: #333; }`,
      ...
      taskMd:
        "🔧 **Расскажи о своём магазине.** Замени в **index.html** две метки:\n\n- заголовок раздела (например «О нас» или «Почему мы?»);\n- 2-3 предложения: когда открылись, что продаёте, почему у вас классно.",
      hintMd:
        "Пиши простыми словами, как будто рассказываешь другу. Например: «Мы открылись в 2026 году и продаём только самые крутые гаджеты. У нас быстрая доставка и низкие цены!»",
      doneMd:
        "✅ **Готово, когда** на сайте появился раздел с заголовком и рассказом о магазине.",
    },
    {
      ...
      time: "12 мин",
      goalMd:
        "**Цель:** добавить отзывы — что о магазине говорят довольные покупатели.\n\n**Повторяем термин:** снова используем `class` — у каждого отзыва класс `review`, и CSS красит их одинаково.",
      actionMd:
...
.review-author { font-weight: bold; color: #5b3df5; margin: 8px 0 0; }`,
      ...
      taskMd:
        "🔧 **Придумай три отзыва.** В каждом отзыве две метки — текст отзыва и имя покупателя. Напиши то, что сказал бы довольный покупатель!\n\nНапример: «Заказал наушники — пришли за один день, очень доволен!» — Артём.",
      hintMd:
        "Кавычки-«ёлочки» уже стоят вокруг текста, а тире — перед именем. Тебе нужно вписать только текст внутри меток `[…]`.",
      doneMd:
        "✅ **Готово, когда** видны три карточки отзывов — с текстом и именами.",
    },
    {
      // ----- Шаг 6: Подвал -----
...
.site-footer p { margin: 4px 0; }`,
      ...
      taskMd:
        "🔧 **Впиши контакты магазина** (можно выдуманные) — четыре метки в **index.html**:\n\n- телефон (например 8-800-555-35-35);\n- почту (например shop@example.com);\n- адрес (например г. Москва, ул. Весёлая, 1);\n- название магазина в копирайте (то же, что в шапке).",
      hintMd: "Меняй только текст внутри меток `[…]`. Значки 📞 ✉️ 📍 и слова «Телефон:», «Почта:» трогать не нужно.",
      doneMd:
        "✅ **Готово, когда** внизу сайта виден подвал с телефоном, почтой, адресом и копирайтом.",
    },
    {
...
buttons.forEach(function (button) {
  button.addEventListener("click", function () {
    ...
    console.log("[ВПИШИ СООБЩЕНИЕ, НАПРИМЕР: Товар добавлен!]");
  });
});`,
      },
      taskMd:
        "🔧 **Впиши своё сообщение.** В **main.js** найди метку `[ВПИШИ СООБЩЕНИЕ …]` внутри `console.log(\"…\")` и замени её на свой текст — например «Ура, покупка!».\n\nПотом нажми ▶ Запустить и понажимай «В корзину»: число у 🛒 будет расти, а в консоли 🖥 внизу появятся твои сообщения.",
      hintMd:
        "Не стирай кавычки `\"` вокруг сообщения — меняй только текст между ними (саму метку со скобками). Счётчик в шапке — это `<span id=\"cart-count\">`.",
      doneMd:
