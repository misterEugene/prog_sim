...
**Вставка.** `insertBlock(i)` дописывает сниппеты в нужные редакторы
(`appendToEditor` — append с отбивкой `\n\n` + `updateHighlight`, тот же путь, что
при ручном вводе), помечает шаг в `doneSteps` (Set), зовёт `autosave` +
`updateProgress`, показывает тост «нажми ▶ Запустить». Превью НЕ перерисовывается
автоматически — ребёнок жмёт «Запустить» сам (как в исходной модели).
