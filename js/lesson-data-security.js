...
**в) Логин автора в комментарии.** Тут чинить уже ничего не надо — эту дыру ты
закрыл в самом начале, когда заменил функцию \`renderComments\`. Но важно понять,
**где именно** в комментариях пряталась опасность. Вредный код может скрываться не
в **тексте** комментария, а в **логине** того, кто его написал. Смотри: злоумышленник
регистрируется под «вредным» логином \`<img src=x onerror="alert(1)">\`, а потом
оставляет самый обычный, безобидный на вид комментарий — например, «классный товар!».
Сайт же рядом с комментарием показывает **имя автора** (его логин). Если бы это имя
выводилось через \`innerHTML\`, код сработал бы у **каждого**, кто просто откроет
страницу товара, — даже если сам комментарий совершенно безобидный. Именно поэтому
в безопасной версии \`renderComments\` мы выводим \`c.author\` через \`textContent\` —
чтобы логин автора всегда показывался как обычный текст.
