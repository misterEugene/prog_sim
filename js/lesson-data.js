...
.banner-btn:hover { opacity: 0.9; }`,
      ...
      taskMd: `**1.** Нажми «Вставить HTML» ⬇ → ▶ Запустить → заполни метки (после каждой — ▶ Запустить):
...
**5.** \`[ВПИШИ РАЗМЕР]\` (размер заголовка) → **44px** → ▶ Запустить.

**6.** \`[ВПИШИ ЦВЕТ]\` (цвет кнопки) → **#ff5722** → ▶ Запустить.

**7. 💪 А теперь сам!** В правиле \`.banner-btn { … }\` найди строку-комментарий
