...
.about p {
	font-size: 17px;
	color: #333;
	/* 👇 НАПИШИ СВОЙСТВО САМ (см. пункт 5 в задании) */
}`,
      },
      taskMd: `**1.** Нажми «Вставить HTML» ⬇ → ▶ Запустить.
...
просторнее. В правиле \`.about p { … }\` под строкой \`/* 👇 НАПИШИ СВОЙСТВО САМ … */\`
добавь своё свойство:
\`line-height: 1.7;\`
...
**2.** Найди строку \`console.log("[ВПИШИ СООБЩЕНИЕ]");\` и замени
\`[ВПИШИ СООБЩЕНИЕ]\` на текст: **Товар добавлен в корзину!**
(кавычки \`"\` НЕ стирай — меняй только текст между ними) → ▶ Запустить.

**3.** Понажимай «В корзину»: число у 🛒 растёт, а в консоли 🖥 внизу появляются
твои сообщения.

**4. 🧪 Поэкспериментируй!** Поменяй \`count = count + 1;\` на \`count = count + 2;\`
