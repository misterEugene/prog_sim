...
**3.** Теперь добавим абзац \`<p>Тут будут самые крутые товары!</p>\` под
заголовком \`<h1>Мой магазин</h1>\`.

Сначала сделай для него **новую пустую строку**: поставь курсор сразу **после
закрывающего тега** \`</h1>\` (в самом конце этой строки) и нажми **Enter** —
появится пустая строка ниже.

⚠️ **Очень важно:** новый абзац должен оказаться **внутри** \`<body>\`, то есть
**выше** закрывающего тега \`</body>\`. Если строка с \`</body>\` оказалась над твоим
абзацем — значит, ты написал его не в том месте.

Напечатай на новой строке этот абзац → ▶ Запустить.`,
      hintMd:
