...
- 1️⃣ Сначала ты **вставляешь блоки кнопками** и заполняешь жёлтые метки \`[ВПИШИ …]\`.
- 2️⃣ Потом — **пишешь теги руками** (как настоящий программист).
...
💡 Видишь на сайте надпись \`[ВПИШИ …]\`? Значит, там ты ещё не вписал текст.
Ошибся — жми **Ctrl+Z**, действие отменится.

🔒 А ещё в конце ты построишь **вход в личный кабинет** и **комментарии**. Запомни
их: на **следующем уроке** ты станешь *этичным хакером* и будешь искать дыры в
этом самом сайте!
...
**Сначала нажми кнопку «Вставить HTML»**, потом ▶ Запустить — увидишь на странице
...
**Теперь нажми кнопку «Вставить CSS»**, ▶ Запустить — блок поменялся, но цвет
...
.banner-btn:hover { opacity: 0.9; }`,
      ...
      taskMd: `🔧 **Вставь HTML → ▶ Запустить → заполни метки** (после каждой — ▶ Запустить):

**1.** \`[ВПИШИ ЗАГОЛОВОК СКИДКИ]\` → **Скидки до 50% на технику!**

**2.** \`[ВПИШИ ПОДЗАГОЛОВОК]\` → **Только до конца недели — успей купить!**

**3.** \`[ВПИШИ ТЕКСТ КНОПКИ]\` → **Купить сейчас**

**Теперь вставь CSS → ▶ Запустить → заполни метки:**

**4.** \`[ВПИШИ РАЗМЕР]\` (размер заголовка) → **44px**

**5.** \`[ВПИШИ ЦВЕТ]\` (цвет кнопки) → **#ff5722**
...
.add-to-cart:hover { background: #4a2fd0; }`,
      ...
      taskMd: `🔧 **Вставь HTML, заполни 4 карточки** (после каждой пары — ▶ Запустить):
...
\`[ВПИШИ ЦЕНУ]\` → число (значок \`₽\` уже стоит справа).

**Вставь CSS → заполни метки:**
...
.about p {
	...
	/* 👇 НАПИШИ СВОЙСТВО САМ (см. пункт 4 в задании) */
      ...
      taskMd: `🔧 **Вставь HTML → ▶ Запустить → заполни метки:**
...
**Вставь CSS → ▶ Запустить → заполни метку:**
...
.review-author { font-weight: bold; color: #5b3df5; margin: 8px 0 0; }`,
      ...
      taskMd: `🔧 **Вставь HTML, впиши три отзыва** (после каждого — ▶ Запустить):
...
.site-footer p { margin: 4px 0; }`,
      ...
      taskMd: `🔧 **Вставь HTML → ▶ Запустить → впиши контакты** (после каждой — ▶ Запустить):
...
**Вставь CSS → ▶ Запустить → заполни метку:**
**5.** \`[ВПИШИ ЦВЕТ]\` (фон подвала) → **#2b2b3a**`,
      hintMd:
...
buttons.forEach(function (button) {
	button.addEventListener("click", function () {
      ...
      taskMd: `🔧 **Вставь JS → впиши сообщение → ▶ Запустить → жми «В корзину».**
...
if (location.hash) {
	...
	helloBox.innerHTML = "Привет, " + fromUrl + "!";  // отражаем кусок ссылки
      ...
      taskMd: `🔧 **Вставь HTML → CSS → JS** (кнопки активируются по очереди). Заполни метки:

**1.** \`[ВПИШИ ЗАГОЛОВОК]\` → **Личный кабинет**

**2.** \`[ВПИШИ ТЕКСТ КНОПКИ]\` → **Войти**

**3.** \`[ВПИШИ ЦВЕТ]\` (цвет заголовка, style.css) → **#5b3df5**

**4.** Нажми ▶ Запустить. Попробуй войти: логин **admin**, пароль **megapass123** —
появится «Привет, admin!». Это твой рабочий вход. 🔑
...
**1.** \`[ВПИШИ ЗАГОЛОВОК]\` → **Комментарии посетителей**

**2.** \`[ВПИШИ ТЕКСТ КНОПКИ]\` → **Отправить**

**3.** \`[ВПИШИ ЦВЕТ]\` (цвет заголовка, style.css) → **#5b3df5**

**4.** \`[ВПИШИ СООБЩЕНИЕ]\` (main.js) → **Новый комментарий добавлен!**

**5.** Нажми ▶ Запустить, напиши пару комментариев и нажми «Отправить» — они
