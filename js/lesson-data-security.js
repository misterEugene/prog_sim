...
ℹ️ **«А почему нельзя просто вставить ссылку в адресную строку браузера?»** Отличный
вопрос — и тут прячется важная вещь. Здесь, на учебной платформе, твой сайт работает
в защищённом **«окошке-песочнице»** (его называют iframe): у него свой собственный
внутренний адрес, поэтому \`#welcome=…\`, дописанный в адресную строку \`index2.html\`,
до сайта **не доходит**. Вдобавок браузер в адресной строке заменяет символы \`<\` и
\`>\` на «коды» вида \`%3C\` и \`%3E\`. Поэтому здесь «ссылку» мы доставляем сайту
**командой в консоли** — так она попадает прямо внутрь песочницы.

А вот на **настоящем** сайте (без песочницы) это сработало бы именно через ссылку:
злоумышленник кидает жертве адрес с \`#welcome=<img …>\`, жертва щёлкает — её браузер
сам раскодирует символы и выполнит чужой код. Именно поэтому отражённый XSS так
опасен: жертве достаточно **просто открыть ссылку**.
