...
\`function renderComments(id)\`. Эта же строка **5 раз** встречается здесь, в тексте
урока (в левой колонке), поэтому нажми **Enter** **6 раз** подряд — браузер будет
перепрыгивать с одной подсветки на следующую, и только на **пятый раз** подсветка
встанет на нужную строку в коде, в средней колонке. Так ты быстро находишь нужное
место, не листая вручную.
...
эту функцию. (Это сочетание встречается в тексте урока **13 раз**, поэтому нажми
**Enter 14 раз** подряд — подсветка будет перескакивать с одной на следующую, и
только на последнем разе встанет на нужную строку в коде.) Выдели её целиком тем же способом (щёлкни в начале строки
...
**Enter**. (Это сочетание встречается в тексте урока **3 раза**, поэтому нажми
**Enter 4 раза** подряд, чтобы подсветка дошла до нужной строки в коде.) Ты найдёшь
строку:
...
**3 раза**, поэтому нажми **Enter 4 раза** подряд, чтобы подсветка дошла до нужной
строки в коде.) Ты найдёшь строку:
...
сочетание встречается в тексте урока **5 раз**, поэтому нажми **Enter 6 раз** подряд,
чтобы подсветка дошла до нужной строки в коде.) Браузер подсветит строки:
...
**12 раз**, поэтому нажми **Enter 13 раз** подряд, чтобы подсветка дошла до нужной
строки в коде.) Этот же пароль ещё и подсказан в комментарии.
...
найди \`megapass123\` (это сочетание встречается в тексте урока **12 раз**, поэтому нажми
**Enter 13 раз** подряд, чтобы подсветка дошла до нужной строки в коде) — это
строка-комментарий вида \`<!-- TODO … megapass123 -->\`.
...
**3 раза**, поэтому нажми **Enter 4 раза** подряд, чтобы дойти до нужной строки в коде),
выдели её целиком (щелчок в начало,
...
\`megapass123\`. (Внимание: это сочетание всё ещё **12 раз** встречается в самом тексте
урока — это нормально, не пугайся. Главное — нажми **Enter 13 раз**: на 13-й раз
подсветка должна была бы прыгнуть в код, но теперь там этого слова **нет**, и подсветка
просто вернётся в начало текста урока.) То же проверь на вкладке **main.js**. И войти
...
\`Пароли не совпадают\`. (Это сочетание встречается в тексте урока **3 раза**, поэтому
нажми **Enter 4 раза** подряд, чтобы подсветка дошла до нужной строки в коде.) Щёлкни
левой кнопкой мыши **в самом конце** этой строки, нажми
...
(Это сочетание встречается в тексте урока **4 раза**, поэтому нажми **Enter 5 раз**
подряд, чтобы подсветка дошла до нужной строки в коде.)
...
**Ctrl+F**, найди \`admin-panel\`. (Это сочетание встречается в тексте урока **10 раз**,
поэтому нажми **Enter 11 раз** подряд, чтобы подсветка дошла до нужной строки в коде.)
Ты увидишь блок из трёх строк: открытие
...
\`showWelcome\` (это сочетание встречается в тексте урока **6 раз**, поэтому нажми
**Enter 7 раз** подряд, чтобы подсветка дошла до нужной строки в коде) — это та самая
...
**6 раз**, поэтому нажми **Enter 7 раз** подряд, чтобы дойти до нужной строки в коде),
выдели её целиком (щелчок в начало строки
...
напечатай \`FLAG\`, нажми **Enter**. (Это сочетание встречается в тексте урока **16 раз**
— поиск не различает большие и маленькие буквы, поэтому нажми **Enter 17 раз** подряд,
чтобы подсветка дошла до нужной строки в коде.) Браузер подсветит строку-комментарий
\`<!-- FLAG{…} -->\` рядом с подвалом сайта (\`<footer>\`). Вот первый!

**🚩 Флаг 2 — в коде JavaScript.** Щёлкни вкладку **main.js**, нажми **Ctrl+F**, снова
напечатай \`FLAG\`. (Это сочетание снова встречается в тексте урока **16 раз**, поэтому
нажми **Enter 17 раз** подряд, чтобы подсветка дошла до нужной строки в коде.) Найдёшь
строку \`var hiddenFlag = "FLAG{…}"\`. Второй!
