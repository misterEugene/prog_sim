...
- \`const maxLen = 4;\` - какой длиной **закончить**. Хочешь проверить пароли до 5 или
  6 цифр - поставь \`5\` или \`6\`. Осторожно: каждая лишняя цифра увеличивает перебор в
  10 раз (6 цифр - это уже миллион вариантов), так что перебор пойдёт заметно дольше -
  как раз и увидишь прогресс каждую секунду.
...
4. Впиши своё - например \`petya\`. **Кавычки \`"\` вокруг логина не удаляй!** А у чисел
   (\`minLen\`, \`maxLen\`) кавычек нет - там просто цифра, например \`const maxLen = 6;\`.
...
всю функцию (щелчок в начало \`function showWelcome(name) {\`, Shift+щелчок после её
\`}\`), нажми **Delete** и сразу **Ctrl+V**:
...
\`location.hash = "welcome=Аня"\` по-прежнему приветствует. Дыра закрыта! 🛡️`,
      hintMd: `Сломать: «🔗 Открыть в новой вкладке» → в адрес ТОЙ вкладки допиши в конец \`#welcome=<img src=x onerror=alert('Взлом!')>\` → Enter (браузер закодирует символы - это норма). Запасной способ - та же строка через F12 → Console: \`location.hash = "welcome=…"\`. Починить: функцию \`showWelcome\` - \`innerHTML\` → \`textContent\`.`,
